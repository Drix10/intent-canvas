...
import { Activity, AlertTriangle, CheckCircle2, CreditCard, ExternalLink, RefreshCw, ShieldCheck } from 'lucide-react';
import { DodoConnectionStatus, DodoSignal, RecoveryCase, RevenueOperationsOverview } from '../../types/canvas';
import { DodoConnectionCard } from './DodoConnectionCard';

interface RecoveryCasePanelProps {
  cases: RecoveryCase[];
  signals: DodoSignal[];
  overview: RevenueOperationsOverview | null;
...
export const RecoveryCasePanel: React.FC<RecoveryCasePanelProps> = ({ cases, signals, overview, connection, loading, importingHistory, historyProgress, onRefresh, onImportHistory, onApprove, onAddEvidence, onAddSignalEvidence }) => {
  const [expanded, setExpanded] = useState<string | null>(null);
  return <aside className="w-[380px] max-w-[calc(100vw-2rem)] rounded-2xl border border-sky-400/25 bg-[#090a0f]/95 p-3 shadow-2xl backdrop-blur-2xl">
    <div className="mb-3 flex items-start justify-between gap-3">
      ...
      <div className="flex gap-1"><button type="button" onClick={onRefresh} aria-label="Refresh Dodo recovery cases" className="rounded-lg border border-white/10 p-1.5 text-neutral-300 hover:bg-white/10"><RefreshCw className={`h-3.5 w-3.5 ${loading ? 'animate-spin' : ''}`} /></button></div>
    </div>
    <DodoConnectionCard connection={connection} loading={loading} importing={importingHistory} historyProgress={historyProgress} onImportHistory={onImportHistory} />
    {overview && <section className="mb-3 rounded-xl border border-sky-400/15 bg-sky-400/[0.04] p-2.5"><div className="flex items-center justify-between gap-2"><p className="flex items-center gap-1 text-[10px] font-bold text-sky-100"><Activity className="h-3.5 w-3.5" />Business signal map</p><span className="text-[9px] text-neutral-500">{overview.retainedSignalCount} retained</span></div><div className="mt-2 grid grid-cols-3 gap-1.5 text-center"><div className="rounded-lg bg-rose-400/10 px-1 py-1.5"><strong className="block text-xs text-rose-100">{overview.caseCounts.detected + overview.caseCounts.escalated}</strong><span className="text-[8px] text-rose-200/75">needs review</span></div><div className="rounded-lg bg-amber-400/10 px-1 py-1.5"><strong className="block text-xs text-amber-100">{overview.caseCounts.approved}</strong><span className="text-[8px] text-amber-200/75">approved</span></div><div className="rounded-lg bg-[#00ff87]/10 px-1 py-1.5"><strong className="block text-xs text-[#b8ffd9]">{overview.signalCounts.recovery}</strong><span className="text-[8px] text-[#b8ffd9]/75">recoveries</span></div></div>{overview.eventFamilies.length > 0 && <p className="mt-2 truncate text-[9px] text-neutral-400">Watching: {overview.eventFamilies.map(item => `${item.family} (${item.count})`).join(' · ')}</p>}</section>}
